...
async function injectHome(){
  try{
    ...
    const old=host.querySelector('.forecastHomeCard'),wrap=document.createElement('div');wrap.innerHTML=homeMarkup();const card=wrap.firstElementChild;
    if(old)old.replaceWith(card);else{const pred=host.querySelector('.hfPrediction')?.closest('.hfCard')||host.querySelector('.hfLiveSlot');if(pred)pred.insertAdjacentElement('afterend',card);else host.appendChild(card)}
...
function noticeMarkup(){
  ...
  return `<div class="noticebox rules forecastNotice">
    <h3>${esc(title)}</h3>
    <p>${esc(body)}</p>
    <div class="forecastNoticeMeta">${locked?'Entries closed':`${esc(countdown(data.competition?.lock_at))} until final lock`} · 5 pts exact · 4 pts ±1 · 3 pts ±2 · 2 pts ±3 · 1 pt ±4</div>
    <div class="noticeaction forecastNoticeBtn"><button class="btn link small" data-notice-forecast>${esc(button)}</button></div>
  </div>`;
}

async function injectNotice(){
  try{
    if(!document.querySelector('#nav button[data-v="home"].active'))return;
    ...
    noticeCard.querySelector('.forecastNotice')?.remove();

    const empty=[...noticeCard.querySelectorAll(':scope > .notice')].find(el=>
    ...
    if(section)section.insertAdjacentElement('afterend',item);
    else noticeCard.prepend(item);

    item.querySelector('[data-notice-forecast]')?.addEventListener('click',openForecast);
  }catch(e){console.warn('Forecast notice:',e)}
}

function schedule(){clearTimeout(timer);timer=setTimeout(()=>{ensureNav();injectHome();injectNotice()},180)}
...
setInterval(()=>{if(document.querySelector('.fcOverlay')){data=null;loadedAt=0;load(true).then(()=>{if(data?.entry?.predicted_order&&!dirty)order=[...data.entry.predicted_order];render()}).catch(()=>{})}else{injectHome();injectNotice()}},60000);
setTimeout(async()=>{try{await load(false);ensureNav();injectHome();injectNotice()}catch(e){console.warn('Forecast startup:',e)}},700);
