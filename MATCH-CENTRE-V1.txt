// MATCH CENTRE V1 - Locked In -> Live -> FT Confirming -> Final + As It Stands
...
function mcCss(){
  if(document.getElementById('mc-v1-css'))return;
...
async function mcFetch(force=false){
  ...
  try{
    const [feedSet,histSet,ctxSet]=await Promise.allSettled([
      mcSb.functions.invoke('live-score-feed'),
...
async function mcRender(force=false){
  ...
  try{
    const d=await mcFetch(force);if(!d)throw Error('Match Centre not ready');
    const rows=d.rows||[];
    ...
    body.innerHTML=`${mcSection(rows,['live','syncing'],'🔴 Live Now','Scores refresh automatically')}${mcSection(rows,['confirming'],'🏁 FT · Confirming','Provider says finished')}${mcSection(rows,['locked'],'🔒 Locked In','Predictions are sealed')}${mcProjectedTable(d)}${mcSection(rows,['final'],'✓ Recently Final','Officially confirmed')}<div class="mcFooter">Match Centre refreshes about every 15 seconds while open. Locked predictions are shown only after the prediction deadline. Provisional points are never written to the official table.</div>`;
...
async function mcOpen(){
  ...
  ov.querySelector('.mcClose').onclick=mcClose;document.body.appendChild(ov);document.body.style.overflow='hidden';await mcRender(true);
}
function mcClose(){
...
async function mcSync(force=false){
  try{const d=await mcFetch(force);if(!d)return;mcEnsureNav(d);mcHome(d);if(document.querySelector('.mcOverlay'))await mcRender(false)}catch(e){console.warn('Match Centre sync:',e)}
}
...
window.addEventListener('focus',()=>mcSync(true));
...
setInterval(()=>mcSync(true),15000);
setTimeout(()=>mcSync(true),600);
