// MATCH CENTRE V3 - resilient locked/live matchday experience
...
async function mc3Fetch(force=false,providerForce=false){
  ...
  mc3FetchPromise=(async()=>{
    ...
    const histRes=await mc3Sb.rpc('get_league_history',{p_league_id:mc3LeagueId,p_limit:200});
    if(histRes.error)throw histRes.error;

    let ctx={standings:[],ai_picks:[],current_user_id:mc3UserId};
    try{
      const r=await mc3Sb.rpc('get_match_centre_context',{p_league_id:mc3LeagueId});
      if(!r.error&&r.data)ctx={...ctx,...r.data};
    }catch(e){
      console.warn('Match Centre V3 context enhancement unavailable:',e);
    }

    let feed={ok:false,live:[],recent_final:[],source:'history',source_label:'Prediction history'};
    try{
      const r=await mc3Sb.functions.invoke('live-score-feed',{body:{force:providerForce}});
      if(!r.error&&r.data?.ok===true)feed=r.data;
    }catch(e){
      console.warn('Match Centre V3 live feed unavailable:',e);
    }
