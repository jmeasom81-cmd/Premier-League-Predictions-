...
function schedule(){clearTimeout(timer);timer=setTimeout(()=>{ensureNav();injectHome()},180)}
...
setInterval(()=>{if(document.querySelector('.fcOverlay')){data=null;loadedAt=0;load(true).then(()=>{if(data?.entry?.predicted_order&&!dirty)order=[...data.entry.predicted_order];render()}).catch(()=>{})}else injectHome()},60000);
setTimeout(async()=>{try{await load(false);ensureNav();injectHome()}catch(e){console.warn('Forecast startup:',e)}},700);
