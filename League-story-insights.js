...
async function tryRender(){
  ...
  if(host.querySelector('.lsiV3Host'))return;

  let d;
  try{d=await loadData()}
  catch(e){
    console.warn('League Story V3:',e);
    return;
  }
  if(!d)return;
  ...
  if(!currentHost||!currentRecords||currentHost.querySelector('.lsiV3Host'))return;

  const a=analyse(d);
  const wrap=document.createElement('div');
  wrap.className='lsiV3Host';
