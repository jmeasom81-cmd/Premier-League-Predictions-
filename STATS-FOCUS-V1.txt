  ...
  return `
    <div class="sfCard sfHero">
      ...
      <div class="sfGrid4">
        ...
        ${sfStat(totalExacts,'Exact scores')}
      </div>
...
function sfHideLegacy(header){
  ...
  if(!main)return;
  main.querySelectorAll('.ljs2-playerHost,.sdHost').forEach(x=>x.style.display='none');
  [...main.children].forEach(el=>{
    ...
    if(['your season','season leaders','who’s hot?','who\'s hot?','who’s not?','who\'s not?','records','league insights','humans vs ai'].includes(title))el.style.display='none';
  });
}
...
async function sfRender(host){
  ...
  try{
    ...
    if(sfMode==='self'){
      ...
      host.innerHTML=sfTabs()+sfLeagueHtml(null);
...
if(sfMain)new MutationObserver(()=>sfSchedule(false,260)).observe(sfMain,{childList:true,subtree:true});
window.addEventListener('focus',()=>{if(sfActive()){sfLoadedAt=0;sfEvidenceAt=0;const h=document.querySelector('.sfHost');if(h)h.dataset.sfRendered='0';sfSchedule(true,180)}});
setInterval(()=>{if(sfActive()){sfLoadedAt=0;const h=document.querySelector('.sfHost');if(h)h.dataset.sfRendered='0';sfSchedule(true,250)}},60000);
